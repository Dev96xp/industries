...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function mount(Contractor $contractor): void
    {
        ...
        $this->companyName    = $contractor->company_name;
        $this->contactName    = $contractor->contact_name ?? '';
        $this->phone          = $contractor->phone ?? '';
        $this->phoneSecondary = $contractor->phone_secondary ?? '';
        $this->email          = $contractor->email ?? '';
        $this->address        = $contractor->address ?? '';
        $this->specialty         = $contractor->specialty ?? '';
        $this->insuranceCompany       = $contractor->insurance_company ?? '';
        $this->hasConstructionLicense = $contractor->has_construction_license;
        $this->licenseNumber          = $contractor->license_number ?? '';
        $this->notes                  = $contractor->notes ?? '';
        $this->isActive               = $contractor->is_active;
    }
    ...
    public function save(): void
    {
        ...
        session()->flash('success', "Contractor \"{$this->companyName}\" updated.");
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-1 sm:p-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <flux:heading size="xl">Edit Contractor</flux:heading>
            <flux:text class="mt-1 text-zinc-500">{{ $contractor->company_name }}</flux:text>
        </div>
    </div>
